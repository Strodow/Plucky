...
# Globals to store the dimensions of the currently initialized video mode
g_active_width = 0
g_active_height = 0
...
def initialize_sdk():
    ...
    # Default api_version_str for all return paths, ensuring two values are always returned.
    api_version_to_return = "N/A"

    if not decklink_dll:
        if not load_dll():
            print("DeckLink DLL not loaded and failed to load, cannot initialize SDK.", file=sys.stderr)
            return False, api_version_to_return
    ...
    # If we reach here, InitializeDLL was S_OK.
    ...
    if hasattr(decklink_dll, "GetAPIVersion"):
        ...
        if hr_ver == S_OK:
            ...
            api_version_to_return = f"{major}.{minor}.{sub}" # Update if successful
            print(f"DeckLink API Version: {api_version_to_return} (Raw: {version_val:#010x})")
            ...
            # api_version_to_return remains "N/A"
    else:
        print("GetAPIVersion function not found in DLL. Cannot retrieve API version.")
        # api_version_to_return remains "N/A"

    print("DeckLink DLL (API level) Initialized successfully.")
    sdk_initialized_successfully = True
    return True, api_version_to_return
...
def initialize_selected_devices(fill_device_idx: int, key_device_idx: int, video_mode_details: dict) -> bool:
    global decklink_initialized_successfully, g_active_width, g_active_height
    if not sdk_initialized_successfully:
    ...
    if hr == S_OK:
        ...
        print(f"Outputs configured for {g_active_width}x{g_active_height} @ {fr_num}/{fr_den} FPS (Num/Den).")
...
def shutdown_selected_devices():
    ...
    if decklink_dll and decklink_initialized_successfully:
        ...
        g_active_width = 0 # Reset active dimensions
        g_active_height = 0
...
def send_external_keying_frames(fill_bgra_bytes, key_bgra_bytes):
    ...
    if g_active_width == 0 or g_active_height == 0:
        print("Error: DeckLink active dimensions not set (or zero). Initialize device first with valid dimensions.", file=sys.stderr)
        return False
        
    expected_frame_size = g_active_width * g_active_height * 4
...
class DeckLinkGUI(QWidget):
    ...
    def on_init_devices(self):
        ...
        test_video_mode_details = {
            "name": f"{g_active_width if g_active_width else DLL_WIDTH}x{g_active_height if g_active_height else DLL_HEIGHT} @ {TARGET_FRAME_RATE_NUM/TARGET_FRAME_RATE_DEN:.2f} (Test GUI Default)",
            "width": g_active_width if g_active_width else DLL_WIDTH, # Use active if set, else default
            "height": g_active_height if g_active_height else DLL_HEIGHT, # Use active if set, else default
            "fr_num": TARGET_FRAME_RATE_NUM, # Example frame rate
            "fr_den": TARGET_FRAME_RATE_DEN  # Example frame rate
        }
    ...
    def on_send_color(self, qcolor: QColor, is_fill_target: bool):
        ...
        # Use active dimensions for creating test color frames for the GUI
        current_w = g_active_width if g_active_width > 0 else DLL_WIDTH
        current_h = g_active_height if g_active_height > 0 else DLL_HEIGHT


        fill_data, key_data = create_solid_color_fill_and_key_images(current_w, current_h, fill_c, key_c)
